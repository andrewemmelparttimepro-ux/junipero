...
On each wake:
1. Read the task board — find every task where **Owner = Bart** and **Status = Ready**.
...
[
  ...
  },
  {
    ...
    "task_id": "TASK-042",
    "field": "Notes",
    "value": "Research complete. Three sources cross-referenced, contradictions documented. Handing back to Thrawn.",
    "agent": "Bart"
  }
]
```
...
1. Pick up tasks where Owner = Bart and Status = Ready. Ignore everything else.
...
5. Minimum three sources per claim. Document contradictions explicitly. State confidence levels.
